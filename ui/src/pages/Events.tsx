...
export default function Events() {
  ...
  useEffect(() => {
    ...
    const lastId = liveEvents.length > 0 ? liveEvents[liveEvents.length - 1].id : undefined
    const url = `/hosts/${hostId}/events/stream${lastId ? `?last_id=${lastId}` : ''}`
            ...
            <Table.Tbody>
              {filtered.map((e) => (
                <Table.Tr key={e.id}>
                  ...
                  <Table.Td style={{ fontSize: 12 }}>{e.protocol}</Table.Td>
