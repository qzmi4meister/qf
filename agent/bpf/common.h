...
/* EVAL_MAX_RULES is set by the build system via -DEVAL_MAX_RULES=N.
 * Compat build (kernel 5.15–5.16): 32  — bounded loop, fits verifier state.
 * Full build   (kernel 5.17+):     64  — bpf_loop(), no verifier limit.
 * Fallback to 32 if not defined so a plain `clang tc_filter.c` still builds. */
#ifndef EVAL_MAX_RULES
#define EVAL_MAX_RULES  32
#endif
