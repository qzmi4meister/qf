...
func (s *AgentServer) Stream(stream qfv1.AgentService_StreamServer) error {
	...
	agentVer := hello.AgentVersion
	kernelVer := hello.KernelVersion
	_ = s.queries.UpdateHostConnected(ctx, storegen.UpdateHostConnectedParams{
		ID:            hostUUID,
		TenantID:      tenantUUID,
	...
	// ── Welcome ──────────────────────────────────────────────────────────────
	cfg := *defaultConfig
	cfg.FlowEventsEnabled = host.FlowEventsEnabled
