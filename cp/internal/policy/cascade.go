...
// OnHostEnrolled computes the initial bundle for a newly enrolled host so
// matching policies are applied on first gRPC connect.
func (cr *CascadeRecompiler) OnHostEnrolled(ctx context.Context, tenantID, hostID string) error {
	slog.Info("cascade: host enrolled, computing initial bundle", "host", hostID)
	return cr.recompileAndDispatch(ctx, tenantID, []string{hostID})
}
